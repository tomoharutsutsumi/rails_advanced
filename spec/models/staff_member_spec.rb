...
RSpec.describe StaffMember, type: :model do
  ...
  describe 'バリデーション' do
    example '@を２個含むemailは無効' do
      pending("gemのバージョンを変える必要あり")
      member = build(:staff_member, email: 'test@@example.com')
      expect(member).not_to be_valid
    end

    example '漢字を含むfamily_name_kanaは無效' do
    ...
    example '他の職員のメールアドレスと重複したemailは有効' do
      member1 = create(:staff_member)
      member2 = build(:staff_member, email: member1)
      expect(member2).not_to be_valid
    end

    example 'ひらがな、カタカナ、漢字、アルファベット以外の文字が氏名に入力された場合無效' do
      member = build(:staff_member, family_name: 33333, given_name: 22222)
      expect(member).not_to be_valid
    end
  end
end
